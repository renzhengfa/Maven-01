...
<body>
	...
	<div class="header">
		...
		<div class="login-registor">
			<ul class="lg">
				<!-- 获取登录的session用户信息 -->
				<li class="login"><a href="javascript:void(0)"> ${cname } </a></li>
				<li class="login"><a
					href="${pageContext.request.contextPath }/mainPageBean?pageindex=1">返回
				</a></li>
			</ul>
		</div>
	</div>
			...
			<a href="${pageContext.request.contextPath }/bookInfo?bid=${book.bid }">
				<div class="book">
					<div class="center-top">
						<!-- 拼写图片路径 -->
						<img alt="img异常" src="${pageContext.request.contextPath }${book.image }" width="100%" height="100%">
					</div>
					<div class="center-bottom">
						<span>单价：${book.price }$</span> <span>作者：${book.author }</span><br /> <span>书名：${book.name }</span>
					</div>
				</div>
			</a>
		</c:forEach>
	...
	<div class="page">
		...
		<ul>
			...
			<c:if test="${pagebean.currentPageIndex > 1 }">
				<li><a href="${pageContext.request.contextPath }/mainPageBeanbycid?index=${1 }&cid=${cid }">首页</a></li>
				<!-- 第一页时首页和上一页不可以点击 -->
				<li><a href="${pageContext.request.contextPath }/mainPageBeanbycid?index=${pagebean.currentPageIndex-1 }&cid=${cid }">&lt;</a></li>
			</c:if>
			<c:forEach begin="1" end="${pagebean.pages }" var="i">
				<c:if test="${pagebean.currentPageIndex == i }">
					<li class="active">
						<a href="${pageContext.request.contextPath }/mainPageBeanbycid?index=${i }&cid=${cid }">
							${i }
						</a>
					</li>
				</c:if>
			...
			<c:if test="${pagebean.currentPageIndex < pagebean.pages }">
				<li><a href="${pageContext.request.contextPath }/mainPageBeanbycid?index=${pagebean.currentPageIndex+1 }&cid=${cid }">&gt;</a></li>
				<!-- 最后一页时尾页和下一页不可以点击 -->
				<li><a href="${pageContext.request.contextPath }/mainPageBeanbycid?index=${pagebean.pages }&cid=${cid }">尾页</a></li>
			</c:if>
		</ul>
	</div>
