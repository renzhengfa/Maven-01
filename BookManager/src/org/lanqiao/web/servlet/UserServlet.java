...
public class UserServlet extends UserBase {
	...
	public void login(HttpServletRequest request, HttpServletResponse response)
		...
		if (identifyingcode.equals(verCode)) {// 验证码正确
			...
			if (user == null) {
				/*
				 * 登录失败,用户名或密码不正确(没有查到有这个人) 调回登录页面继续登录(存一个错误的cookie信息回显给用户)
				 */
				...
				if (status.equals("1")) {
					/*
					 * 登录成功,跳转页面 1.管理员或超级管理员跳到后台管理系统 2.普通用户或vip跳转到前台系统
					 * 
					 * 判断是否是管理员:根据rid判断 (1.普通用户 2.vip 3.管理员 4.超级管理员)
					 */
					...
					if (user.getRid() == 3 || user.getRid() == 4) {
						...
						if (remenberPassword != null) {
							...
							/*
							 * 记住密码没有被√中 不存Session直接跳转到首页 清除Session
							 */
	...
	protected void registor(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
